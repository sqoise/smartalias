...
export default function AddAnnouncementView({ open, onClose, onSubmit, loading = false }) {
  ...
  // Convert ANNOUNCEMENT_TYPE_NAMES to CustomSelect options format
  const typeOptions = Object.entries(ANNOUNCEMENT_TYPE_NAMES).map(([value, label]) => ({
    value,
    label
  }))
    ...
    <>
      <div className="flex items-center justify-end space-x-2">
        <button
          ...
          onClick={onClose}
          disabled={loading}
          className="inline-flex items-center justify-center px-3 py-1.5 text-sm font-medium rounded-md text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors cursor-pointer h-9 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Cancel
        </button>
        <button
  ...
  return (
    <SlidePanel
      ...
      subtitle="Create announcements for residents"
      size="lg"
      headerIcon="bi-plus-lg"
